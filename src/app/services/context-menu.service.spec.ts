...
describe('ContextMenuService', () => {
  ...
  describe('openFor selection rules', () => {
    ...
    it('opening a menu never touches History', () => {
      service.openFor({ kind: 'canvas' }, 10, 10);
      service.openFor({ kind: 'canvas' }, 20, 20);
  ...
  describe('rename / editText requests', () => {
    it('requestRename exposes the Group id to edit and never touches History', () => {
      const group = graphService.createGroup('G', 0, 0);

      service.requestRename(group.id);

      expect(service.renameRequest()).toBe(group.id);
      expect(historyService.canUndo()).toBe(false);
    });

    it('requestRename is a no-op for a regular node (nodes carry Text, not a Label)', () => {
      const node = graphService.createNode('A', 0, 0);

      service.requestRename(node.id);
    ...
    it('requestEditText on a regular node exposes the node id and never touches History', () => {
      const node = graphService.createNode('A', 0, 0);

      service.requestEditText(node.id);
    ...
    it('requestEditText on a Group is a no-op (Groups are renamed, not text-edited)', () => {
      const group = graphService.createGroup('G', 0, 0);

      service.requestEditText(group.id);
    ...
    it('requestConnectionText exposes the connection id and never touches History', () => {
      ...
      service.requestConnectionText(conn.id);
  ...
  describe('clipboard actions', () => {
    it('copySelection fills the Clipboard and exposes canPaste', () => {
      const node = graphService.createNode('A', 0, 0);
      graphService.selectNode(node.id);
    ...
    it('cutSelection removes the set as one undo step', () => {
      const node = graphService.createNode('A', 0, 0);
      graphService.selectNode(node.id);
    ...
    it('copy/cut/duplicateSelection are silent no-ops with no Node selected', () => {
      const a = graphService.createNode('A', 0, 0);
      const b = graphService.createNode('B', 300, 0);
      const conn = graphService.createConnection(a.id, 'right', b.id, 'left')!;
    ...
    it('duplicateSelection staggers the copy +24,+24 without touching the Clipboard', () => {
      const a = graphService.createNode('A', 0, 0);
      const b = graphService.createNode('B', 500, 500);
      graphService.selectNode(a.id);
      service.copySelection();

      graphService.selectNode(b.id);
      service.duplicateSelection();
